...
function CategoryRow({cat,pct,onChange}){
  const [open,setOpen]=useState(false);
  const col=catBandColor(pct), word=catBandWord(pct);
  return(<div style={{border:`0.5px solid ${C.border}`,borderRadius:12,padding:"13px 16px",marginBottom:10,background:C.white}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12}}>
      <div><div style={{fontFamily:"'Nunito',sans-serif",fontSize:14.5,fontWeight:700,color:C.slate}}>{cat.label}</div><div style={{fontFamily:"'Nunito',sans-serif",fontSize:11.5,color:C.slateLight}}>{cat.q}</div></div>
      <div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
        <div style={{textAlign:"right",lineHeight:1.1}}>
          <div><span style={{fontFamily:"'Playfair Display',serif",fontSize:18,fontWeight:700,color:col}}>{Math.round(pct)}</span><span style={{fontFamily:"'Nunito',sans-serif",fontSize:10.5,color:C.slateLight}}>/100</span></div>
          <div style={{fontFamily:"'Nunito',sans-serif",fontSize:11,fontWeight:800,color:col}}>{word}</div>
        </div>
        <button onClick={()=>setOpen(o=>!o)} style={{background:"none",border:"none",cursor:"pointer",color:C.slateLight,fontSize:11,padding:2}}>{open?"▲":"▼"}</button>
      </div>
    </div>
    <input type="range" min="0" max="100" step="1" value={Math.round(pct)} onChange={e=>onChange(parseInt(e.target.value))} onWheel={e=>e.currentTarget.blur()} style={{width:"100%",marginTop:10,accentColor:col,cursor:"pointer"}}/>
    {open&&<div style={{marginTop:8,paddingTop:8,borderTop:`1px solid ${C.cream}`}}>
      {cat.fs.length?cat.fs.map(f=>{const good=f.pts>=f.weight*0.5;return(<div key={f.key} style={{display:"flex",gap:8,alignItems:"flex-start",marginBottom:5}}><span style={{color:good?"#2E7D52":"#C0392B",fontWeight:800,fontSize:13,lineHeight:1.3}}>{good?"✓":"!"}</span><span style={{fontFamily:"'Nunito',sans-serif",fontSize:12.5,color:C.slateLight,lineHeight:1.5}}>{good?gainReason(f):lossReason(f)}</span></div>);}):<p style={{fontFamily:"'Nunito',sans-serif",fontSize:12,color:C.slateLight,margin:0}}>Add more detail above to score this area.</p>}
    </div>}
  </div>);
}
function QualityScorecard({biz, overrides, score, detail}){
  ...
  const byKey={}; factors.forEach(f=>byKey[f.key]=f);
  const cats=SCORE_CATS.map(c=>{const fs=c.keys.map(k=>byKey[k]).filter(Boolean);if(!fs.length)return null;const maxW=fs.reduce((s,f)=>s+f.weight,0);const natPts=fs.reduce((s,f)=>s+f.pts,0);const natPct=Math.max(0,Math.min(100,(natPts+maxW)/(2*maxW)*100));return {...c,fs,maxW,natPts,natPct};}).filter(Boolean);
  const [catOv,setCatOv]=useState({});
  const rawBase=50+factors.reduce((s,f)=>s+f.pts,0);
  let adjRaw=rawBase;
  cats.forEach(c=>{if(catOv[c.key]!=null){const ovPts=c.maxW*(2*catOv[c.key]/100-1);adjRaw+=(ovPts-c.natPts);}});
  const rawClamp=Math.max(10,Math.min(95,rawBase));
  const adjClamp=Math.max(10,Math.min(95,adjRaw));
  const baseScore=Math.round(rawClamp);
  const adjScore=Math.round(adjClamp);
  const baseVal=calcValuation(biz,ov,rawClamp).eqMid;
  const adjVal=calcValuation(biz,ov,adjClamp).eqMid;
  const dVal=adjVal-baseVal;
  const changed=Object.keys(catOv).some(k=>catOv[k]!=null);
  ...
  const weakest=cats.length?[...cats].sort((a,b)=>a.natPct-b.natPct)[0]:null;
  let weakUpl=null;
  if(weakest){const ovPts=weakest.maxW*0.7;const sc=Math.round(Math.max(10,Math.min(95,rawBase+(ovPts-weakest.natPts))));weakUpl=Math.max(0,calcValuation(biz,ov,sc).eqMid-baseVal);}
  const pctOf=c=>catOv[c.key]!=null?catOv[c.key]:c.natPct;
  const up=dVal>1500, dn=dVal<-1500;
  return(<Card>
    <style>{"@keyframes vpulse{0%{transform:scale(0.985)}55%{transform:scale(1.03)}100%{transform:scale(1)}}"}</style>
    <div style={{background:C.navy,borderRadius:12,padding:"20px 22px",marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:14,gap:12,flexWrap:"wrap"}}>
        <div>
          <div style={{fontFamily:"'Nunito',sans-serif",fontSize:11,fontWeight:700,letterSpacing:"0.1em",color:C.goldLight}}>BUSINESS QUALITY</div>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,fontWeight:700,color:C.white,lineHeight:1.1}}>{sw.word} business</div>
        </div>
        <div style={{textAlign:"right"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:36,fontWeight:700,color:sw.dark,lineHeight:1}}>{adjScore}<span style={{fontSize:16,color:"rgba(255,255,255,0.6)"}}>/100</span></div>
          {changed&&adjScore!==baseScore&&<div style={{fontFamily:"'Nunito',sans-serif",fontSize:11,color:"rgba(255,255,255,0.6)",marginTop:2}}>was {baseScore}</div>}
        </div>
      </div>
      <div style={{position:"relative",marginBottom:6}}>
        <div style={{display:"flex",height:14,borderRadius:7,overflow:"hidden"}}>
          <div style={{width:"45%",background:"#B23A2E"}}/><div style={{width:"15%",background:"#C77D12"}}/><div style={{width:"15%",background:"#5a9e6f"}}/><div style={{width:"25%",background:"#2E7D52"}}/>
        </div>
        <div style={{position:"absolute",top:-3,bottom:-3,left:`${pos}%`,width:3,background:C.white,borderRadius:2,transform:"translateX(-1.5px)",transition:"left 0.2s ease"}}/>
      </div>
      <div style={{display:"flex",justifyContent:"space-between",fontFamily:"'Nunito',sans-serif",fontSize:10.5,color:"rgba(255,255,255,0.7)"}}>
        {["Needs work","Average","Good","Strong"].map(t=><span key={t} style={{color:t===sw.word?C.white:"rgba(255,255,255,0.6)",fontWeight:t===sw.word?700:400}}>{t}{t===sw.word?" ●":""}</span>)}
      </div>
      {nextTier?<div style={{fontFamily:"'Nunito',sans-serif",fontSize:13,color:C.white,marginTop:12}}>You're <strong>{ptsToNext} point{ptsToNext===1?"":"s"}</strong> from a <strong>{nextTier.name}</strong> business — the tier that sells toward the top of the range.</div>:<div style={{fontFamily:"'Nunito',sans-serif",fontSize:13,color:C.white,marginTop:12}}>This is a <strong>Strong</strong> business — it sells toward the top of the range.</div>}
    </div>

    <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:16}}>
      <div style={{flex:"1 1 150px",background:C.cream,borderRadius:10,padding:"12px 15px"}}>
        <div style={{fontFamily:"'Nunito',sans-serif",fontSize:10.5,fontWeight:700,letterSpacing:"0.06em",color:C.slateLight}}>TODAY</div>
        <div style={{fontFamily:"'Playfair Display',serif",fontSize:21,fontWeight:700,color:C.slate}}>{fmt(baseVal)}</div>
      </div>
      <div key={Math.round(dVal/500)} style={{flex:"1.5 1 200px",borderRadius:10,padding:"12px 16px",background:up?"linear-gradient(135deg,#1f7a5c,#2E7D52)":dn?"linear-gradient(135deg,#C0392B,#a93226)":C.cream,boxShadow:(up||dn)?"0 6px 20px rgba(31,122,92,0.22)":"none",animation:(up||dn)?"vpulse 0.4s ease":"none"}}>
        <div style={{fontFamily:"'Nunito',sans-serif",fontSize:10.5,fontWeight:700,letterSpacing:"0.06em",color:(up||dn)?"rgba(255,255,255,0.85)":C.slateLight}}>WITH YOUR CHANGES</div>
        <div style={{fontFamily:"'Playfair Display',serif",fontSize:27,fontWeight:700,color:(up||dn)?C.white:C.slate,lineHeight:1.1}}>{fmt(adjVal)}</div>
        {up&&<div style={{fontFamily:"'Nunito',sans-serif",fontSize:13,fontWeight:800,color:"rgba(255,255,255,0.95)",marginTop:2}}>▲ +{fmt(dVal)}</div>}
        {dn&&<div style={{fontFamily:"'Nunito',sans-serif",fontSize:13,fontWeight:800,color:"rgba(255,255,255,0.95)",marginTop:2}}>▼ −{fmt(Math.abs(dVal))}</div>}
      </div>
    </div>

    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,gap:8}}>
      <div style={{fontFamily:"'Nunito',sans-serif",fontSize:12,fontWeight:700,letterSpacing:"0.04em",color:C.slateLight}}>DRAG TO SEE WHAT MOVES YOUR VALUE MOST</div>
      {changed&&<button onClick={()=>setCatOv({})} style={{background:"none",border:`1px solid ${C.creamDark}`,borderRadius:6,padding:"4px 10px",cursor:"pointer",fontFamily:"'Nunito',sans-serif",fontSize:11.5,fontWeight:700,color:C.navy}}>↺ Reset</button>}
    </div>
    {cats.map(c=><CategoryRow key={c.key} cat={c} pct={pctOf(c)} onChange={v=>setCatOv(o=>({...o,[c.key]:v}))}/>)}

    {weakest&&<div style={{background:"linear-gradient(135deg,#1f7a5c,#2E7D52)",borderRadius:12,padding:"16px 18px",display:"flex",alignItems:"center",gap:14,flexWrap:"wrap",marginTop:4}}>
      <div style={{flex:"1 1 200px"}}>
        ...
        <div style={{fontFamily:"'Nunito',sans-serif",fontSize:11.5,color:"rgba(255,255,255,0.8)"}}>Your weakest area — raising it lifts the whole score</div>
      </div>
      {weakUpl>1000&&<div style={{textAlign:"right"}}><div style={{fontFamily:"'Playfair Display',serif",fontSize:22,fontWeight:700,color:C.white}}>+{fmtM(weakUpl)}</div><div style={{fontFamily:"'Nunito',sans-serif",fontSize:11,color:"rgba(255,255,255,0.8)"}}>potential value</div></div>}
    </div>}
  </Card>);
}
