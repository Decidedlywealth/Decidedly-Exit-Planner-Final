...
function ModuleBusiness({biz,setBiz,wealth,setWealth,overrides,setOverrides,kData,isAdvisor}){
  ...
  <div style={{display:"flex",flexDirection:"column",gap:24}}>
    ...
    <div style={{display:"flex",flexWrap:"wrap",gap:16}}>
      ...
      <Field label="% Revenue from Top 3 Customers" half><Input value={biz.customerConc||""} onChange={v=>sb("customerConc",v)} min="0" max="100" placeholder="35" type="number" hint=">40% is a buyer concern"/></Field>
...
function VModuleInputs({biz,setBiz,overrides,setOverrides}){
  ...
  <div style={{display:"flex",flexDirection:"column",gap:24}}>
    ...
    {ready&&(()=>{const v2=calcValuation(biz,overrides);return(<Card style={{background:`linear-gradient(135deg,${C.navy},${C.navyMid})`,border:"none"}}>
      <h3 style={{fontFamily:"'Playfair Display',serif",fontSize:18,color:C.goldLight,marginTop:0,marginBottom:14}}>Estimated Business Value</h3>
      <div style={{marginBottom:18}}>
...
Under 300 words. Honest and specific. Attribute any typical multiple to the benchmark basis rather than stating it as fact.`;
  ...
  <div style={{display:"flex",flexDirection:"column",gap:24}}>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:9}}>{lost.map((f,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderRadius:8,background:"rgba(199,125,18,0.07)",border:"1px solid rgba(199,125,18,0.28)"}}><div style={{flex:1,minWidth:0,fontFamily:"'Nunito',sans-serif",fontSize:13,color:C.slate}}><strong>{f.label}</strong> <span style={{color:C.slateLight}}>· {f.val}</span></div><div style={{fontFamily:"'Nunito',sans-serif",fontSize:13,fontWeight:800,color:"#C77D12",whiteSpace:"nowrap"}}>{f.lost} pts to gain</div></div>))}</div></Card>}
