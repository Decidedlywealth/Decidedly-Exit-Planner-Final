...
function WhatIfSliders({biz, overrides}){
  ...
  return(<Card>
    ...
    <div style={{display:"flex",gap:18,flexWrap:"wrap",alignItems:"stretch",marginBottom:18}}>
      <div style={{flex:"1 1 220px",padding:"18px 20px",borderRadius:12,background:C.cream,border:`1px solid ${C.border}`}}>
        <div style={{fontFamily:"'Nunito',sans-serif",fontSize:11,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",color:C.slateLight,marginBottom:4}}>Where you are now</div>
        <div style={{fontFamily:"'Playfair Display',serif",fontSize:26,fontWeight:700,color:C.slate}}>{fmtM(base.eqMid)}</div>
      </div>
      <div key={Math.round(uplift/500)} style={{flex:"1 1 220px",padding:"18px 20px",borderRadius:12,background:potBg,border:(up||down)?"none":`1px solid ${C.border}`,boxShadow:up?"0 8px 24px rgba(31,122,92,0.34)":down?"0 8px 24px rgba(192,57,43,0.30)":"none",animation:(up||down)?"vpulse 0.42s ease":"none"}}>
        <div style={{fontFamily:"'Nunito',sans-serif",fontSize:11,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",color:(up||down)?"rgba(255,255,255,0.85)":C.slateLight,marginBottom:4}}>What it could be worth</div>
        <div style={{fontFamily:"'Playfair Display',serif",fontSize:26,fontWeight:700,color:(up||down)?C.white:C.slate}}>{fmtM(proj.eqMid)}</div>
        {up&&<div style={{fontFamily:"'Nunito',sans-serif",fontSize:18,fontWeight:800,color:C.white,marginTop:4}}>▲ +{fmtM(uplift)}</div>}
        {down&&<div style={{fontFamily:"'Nunito',sans-serif",fontSize:13.5,fontWeight:800,color:C.white,marginTop:4}}>▼ {fmtM(uplift)} · this is what buyers pay less for</div>}
      </div>
    </div>
    <div style={{...row,paddingTop:0}}>
      <div style={lbl}>1. Reduce how much the business depends on you</div>
      <div style={cur(ov.ownerDep==="high")}>Now: {ownerStops[oIdx][1]}{ov.ownerDep==="high"?"  ⚠ buyers discount this":""}</div>
      <Slider value={oIdx} max={2} step={1} onChange={i=>setOv(o=>({...o,ownerDep:ownerStops[i][0]}))} leftLabel="Mostly me" rightLabel="Runs without me"/>
    </div>
    <div style={row}>
      <div style={lbl}>2. Build more repeat / recurring revenue</div>
      <div style={cur(ov.revenueType==="transactional")}>Now: {revStops[rIdx][1]}</div>
      <Slider value={rIdx} max={3} step={1} onChange={i=>setOv(o=>({...o,revenueType:revStops[i][0]}))} leftLabel="One-off" rightLabel="Recurring"/>
    </div>
    <div style={{...row,borderBottom:"none",paddingBottom:0}}>
      <div style={lbl}>3. Spread revenue across more customers</div>
      <div style={cur(conc>40)}>{conc}% comes from your top 3 customers{conc>40?"  ⚠ buyer concern":""}</div>
      <Slider value={conc} max={80} step={5} onChange={n=>setOv(o=>({...o,customerConc:String(n)}))} leftLabel="Well spread (0%)" rightLabel="Concentrated (80%)"/>
    </div>
  </Card>);
}
...
Under 300 words. Honest and specific. Attribute any typical multiple to the benchmark basis rather than stating it as fact.`;
  ...
  <div style={{display:"flex",flexDirection:"column",gap:24}}>
    ...
    {lost.length>0&&<Card><h3 style={{fontFamily:"'Playfair Display',serif",fontSize:17,color:C.navy,marginTop:0,marginBottom:6}}>What's Holding the Value Back</h3>
    <p style={{fontFamily:"'Nunito',sans-serif",fontSize:13,color:C.slateLight,marginBottom:16,marginTop:0}}>The biggest opportunities to raise the score — and the value — before a sale.</p>
    <div style={{display:"flex",flexDirection:"column",gap:9}}>{lost.map((f,i)=>(<div key={i} style={{padding:"10px 14px",borderRadius:8,background:"rgba(199,125,18,0.07)",border:"1px solid rgba(199,125,18,0.28)"}}><div style={{display:"flex",alignItems:"center",gap:10}}><div style={{flex:1,minWidth:0,fontFamily:"'Nunito',sans-serif",fontSize:13,color:C.slate}}><strong>{f.label}</strong> <span style={{color:C.slateLight}}>· {f.val}</span></div><div style={{fontFamily:"'Nunito',sans-serif",fontSize:13,fontWeight:800,color:"#C77D12",whiteSpace:"nowrap"}}>{f.lost} pts to gain</div></div><div style={{fontFamily:"'Nunito',sans-serif",fontSize:12.5,color:C.slateLight,lineHeight:1.5,marginTop:3}}>{lossReason(f)}</div></div>))}</div></Card>}
