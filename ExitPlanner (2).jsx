...
function ModuleBusiness({biz,setBiz,wealth,setWealth,overrides,setOverrides,kData,isAdvisor}){
  ...
  <div style={{display:"flex",flexDirection:"column",gap:24}}>
    ...
    {biz.revenue&&biz.netIncome&&(()=>{
      ...
      <WhatIfSliders biz={biz} overrides={overrides}/>
      <ValueLab biz={biz} overrides={overrides}/>
      </>);
    })()}
...
function QualityScorecard({biz, overrides, score, detail}){
  ...
  const nextTier = score<45?{name:"Average",at:45}:score<60?{name:"Good",at:60}:score<75?{name:"Strong",at:75}:null;
  const factors=detail.factors||[];
  const isEntered=f=>{const v=(f.val||"").toString().toLowerCase();return !(v===""||v==="—"||v.includes("not entered")||v.includes("enter cogs"));};
  const strengths=factors.filter(f=>isEntered(f)&&f.pts>=f.weight*0.55).sort((a,b)=>b.pts-a.pts);
  const gaps=factors.map(f=>({...f,lost:f.weight-f.pts})).filter(f=>f.pts<=f.weight*0.45).sort((a,b)=>b.lost-a.lost);
  const top=gaps[0];
  const upl=top?factorUplift(biz,overrides||{},top.key):null;
  const ptsToNext=nextTier?Math.max(1,Math.round(nextTier.at-score)):0;
  return(<Card>
    <div style={{background:C.navy,borderRadius:12,padding:"20px 22px",marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:14,gap:12,flexWrap:"wrap"}}>
        <div>
          ...
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,fontWeight:700,color:C.white,lineHeight:1.1}}>{sw.word==="Needs work"?"Needs work":sw.word} business</div>
        </div>
        <div style={{fontFamily:"'Playfair Display',serif",fontSize:36,fontWeight:700,color:sw.dark,lineHeight:1}}>{score}<span style={{fontSize:16,color:"rgba(255,255,255,0.6)"}}>/100</span></div>
      </div>
      <div style={{position:"relative",marginBottom:6}}>
        ...
        <div style={{position:"absolute",top:-3,bottom:-3,left:`${pos}%`,width:3,background:C.white,borderRadius:2,transform:"translateX(-1.5px)"}}/>
      </div>
      ...
      {nextTier&&<div style={{fontFamily:"'Nunito',sans-serif",fontSize:13,color:C.white,marginTop:12}}>You're <strong>{ptsToNext} point{ptsToNext===1?"":"s"}</strong> from a <strong>{nextTier.name}</strong> business — the tier that sells toward the top of the range.</div>}
      {!nextTier&&<div style={{fontFamily:"'Nunito',sans-serif",fontSize:13,color:C.white,marginTop:12}}>This is a <strong>Strong</strong> business — it sells toward the top of the range.</div>}
    </div>

    <div style={{display:"flex",gap:14,flexWrap:"wrap",marginBottom:top?14:0}}>
      <div style={{flex:"1 1 240px"}}>
        <div style={{fontFamily:"'Nunito',sans-serif",fontSize:12,fontWeight:800,color:"#2E7D52",letterSpacing:"0.03em",marginBottom:4}}>WHAT'S LIFTING YOUR VALUE</div>
        {strengths.length>0?strengths.map((f,i)=><ScoreRow key={f.key} icon="✓" color="#2E7D52" label={f.label} detail={gainReason(f)+" ("+f.val+")"} last={i===strengths.length-1}/>):<p style={{fontFamily:"'Nunito',sans-serif",fontSize:12.5,color:C.slateLight,margin:"8px 0 0"}}>Fill in more details above to see your strengths here.</p>}
      </div>
      <div style={{flex:"1 1 240px"}}>
        <div style={{fontFamily:"'Nunito',sans-serif",fontSize:12,fontWeight:800,color:"#C0392B",letterSpacing:"0.03em",marginBottom:4}}>WHAT'S HOLDING IT BACK</div>
        {gaps.length>0?gaps.map((f,i)=><ScoreRow key={f.key} icon="!" color="#C0392B" label={f.label} detail={lossReason(f)} last={i===gaps.length-1}/>):<p style={{fontFamily:"'Nunito',sans-serif",fontSize:12.5,color:C.slateLight,margin:"8px 0 0"}}>Nothing major — this business scores well across the board.</p>}
      </div>
    </div>

    {top&&<div style={{background:"linear-gradient(135deg,#1f7a5c,#2E7D52)",borderRadius:12,padding:"16px 18px",display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
      <div style={{flex:"1 1 200px"}}>
        ...
        <div style={{fontFamily:"'Nunito',sans-serif",fontSize:15,fontWeight:700,color:C.white}}>{SC_ACTION[top.key]||("Improve "+top.label.toLowerCase())}</div>
      </div>
      <div style={{textAlign:"right"}}>
        {upl?<><div style={{fontFamily:"'Playfair Display',serif",fontSize:22,fontWeight:700,color:C.white}}>+{fmtM(upl)}</div><div style={{fontFamily:"'Nunito',sans-serif",fontSize:11,color:"rgba(255,255,255,0.8)"}}>potential value</div></>:<><div style={{fontFamily:"'Playfair Display',serif",fontSize:22,fontWeight:700,color:C.white}}>+{Math.round(top.lost)} pts</div><div style={{fontFamily:"'Nunito',sans-serif",fontSize:11,color:"rgba(255,255,255,0.8)"}}>to your score</div></>}
      </div>
    </div>}
  </Card>);
}
