  ...
  return(<div style={{border:`0.5px solid ${C.border}`,borderRadius:12,padding:"13px 16px",marginBottom:10,background:C.white}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
        <span style={{fontFamily:"'Nunito',sans-serif",fontSize:13,fontWeight:800,color:col}}>{word}</span>
...
function QualityScorecard({biz, overrides, score, detail}){
  ...
  const baseScore=Math.round(Math.max(10,Math.min(95,rawBase)));
  const adjScore=Math.round(Math.max(10,Math.min(95,adjRaw)));
  const baseVal=calcValuation(biz,ov).eqMid;
  const adjVal=calcValuation(biz,ov,adjScore).eqMid;
  ...
  return(<Card>
    ...
    <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:16}}>
      <div style={{flex:"1 1 140px",background:C.cream,borderRadius:10,padding:"12px 15px"}}>
        <div style={{fontFamily:"'Nunito',sans-serif",fontSize:10.5,fontWeight:700,letterSpacing:"0.06em",color:C.slateLight}}>TODAY</div>
        <div style={{fontFamily:"'Playfair Display',serif",fontSize:22,fontWeight:700,color:C.slate}}>{fmtM(baseVal)}</div>
      </div>
      <div key={Math.round(dVal/500)} style={{flex:"1.3 1 175px",borderRadius:10,padding:"12px 15px",background:up?"linear-gradient(135deg,#1f7a5c,#2E7D52)":dn?"linear-gradient(135deg,#C0392B,#a93226)":C.cream,boxShadow:(up||dn)?"0 6px 18px rgba(31,122,92,0.20)":"none",animation:(up||dn)?"vpulse 0.4s ease":"none"}}>
        ...
        <div style={{fontFamily:"'Playfair Display',serif",fontSize:22,fontWeight:700,color:(up||dn)?C.white:C.slate}}>{fmtM(adjVal)} {up&&<span style={{fontSize:14}}>▲ +{fmtM(dVal)}</span>}{dn&&<span style={{fontSize:14}}>▼ {fmtM(dVal)}</span>}</div>
      </div>
    </div>
  ...
  <div style={{display:"flex",flexDirection:"column",gap:24}}>
    ...
    {ready&&(()=>{const v2=calcValuation(biz,overrides);const methods=calcMethods(biz,overrides,v2);return(<>
      ...
      <div style={{marginBottom:16}}>
        ...
        <div style={{fontFamily:"'Nunito',sans-serif",fontSize:12.5,color:"rgba(255,255,255,0.75)",marginTop:8,lineHeight:1.55,maxWidth:620}}>Midpoint estimate {fmtM(v2.eqMid)}. A planning <strong>estimate built from published market benchmarks — not a formal appraisal or an offer.</strong> Speak to the range, not a single number.</div>
      </div>
      ...
      <Card>
        ...
        <p style={{fontFamily:"'Nunito',sans-serif",fontSize:10.5,color:C.slateLight,marginTop:12,marginBottom:0,lineHeight:1.5}}>Capitalization, cap rate, discount rate, and book value use standard planning assumptions an advisor can refine. Methods that need a full balance sheet are marked accordingly.</p>
      </Card>
    </>);})()}
    <Card><h3 style={{fontFamily:"'Playfair Display',serif",fontSize:17,color:C.navy,marginTop:0,marginBottom:16}}>Market Comparables — {ind.label}</h3>
