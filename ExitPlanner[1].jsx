...
const DRV = {
  ...
  recurringPct:{ label:"Recurring revenue %", input:{key:"recurringPct",ph:"70",hint:"Subscription/contract % of total revenue"}, get:b=>{const x=p(b.recurringPct);let f=0;if(x){if(x>=80)f=1;else if(x>=60)f=0.6;else if(x>=40)f=0.1;else if(x>=20)f=-0.3;else f=-0.6;}return {frac:f,val:x?x+"%":"—"};} },
  netRetention:{ label:"Net revenue retention", input:{key:"nrr",ph:"105",hint:"Expansion minus churn; >100% is a premium signal"}, get:b=>{const x=p(b.nrr);let f=0;if(x){if(x>=110)f=1;else if(x>=100)f=0.6;else if(x>=90)f=0;else f=-0.6;}return {frac:f,val:x?x+"%":"—"};} },
  rainmaker:{ label:"Personal-goodwill (rainmaker) dependence", input:{key:"rainmakerDep",ph:"60",hint:"% of revenue tied to owner's personal client relationships — lower is better"}, get:b=>{const x=p(b.rainmakerDep);let f=0;if(x){if(x<=20)f=1;else if(x<=40)f=0.4;else if(x<=60)f=-0.2;else if(x<=80)f=-0.7;else f=-1;}return {frac:f,val:x?x+"% personal":"—"};} },
  maintenance:{ label:"Recurring service/maintenance revenue", input:{key:"maintPct",ph:"25",hint:"% from service/maintenance contracts vs one-time installs"}, get:b=>{const x=p(b.maintPct);let f=-0.1;if(x){if(x>=40)f=1;else if(x>=25)f=0.6;else if(x>=10)f=0.2;else f=-0.1;}return {frac:f,val:x?x+"%":"—"};} },
  publicBacklog:{ label:"Repeat / public-sector client base", input:{key:"repeatPct",ph:"50",hint:"% of revenue from repeat or government clients (stability)"}, get:b=>{const x=p(b.repeatPct);let f=0;if(x){if(x>=60)f=1;else if(x>=40)f=0.5;else if(x>=20)f=0.1;else f=-0.3;}return {frac:f,val:x?x+"%":"—"};} },
  payerMix:{ label:"Private-pay / payer diversification", input:{key:"privatePayPct",ph:"40",hint:"% private-pay vs insurance reimbursement"}, get:b=>{const x=p(b.privatePayPct);let f=0;if(x){if(x>=50)f=1;else if(x>=30)f=0.5;else if(x>=15)f=0.1;else f=-0.3;}return {frac:f,val:x?x+"%":"—"};} },
  ...
  online:{ label:"E-commerce / channel diversification", input:{key:"onlinePct",ph:"30",hint:"% of revenue online (widens the buyer pool)"}, get:b=>{const x=p(b.onlinePct);let f=-0.1;if(x){if(x>=40)f=1;else if(x>=20)f=0.5;else if(x>=10)f=0.1;else f=-0.1;}return {frac:f,val:x?x+"%":"—"};} },
};
...
function HeadlineBanner({wc,yrs,ready,compact}){
  ...
  const levers=["Save about "+fmt(wc.gap/y)+"/yr more until exit","Grow the business so the sale nets more","Adjust timing — delaying the exit adds savings + growth"];
  return(<div style={{borderRadius:14,overflow:"hidden",background:`linear-gradient(135deg,${C.navy},${C.gold})`,boxShadow:"0 8px 30px rgba(6,56,148,0.30)"}}>
    <div style={{padding:compact?"18px 22px":"26px 28px",color:C.white}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:8}}>
        ...
        <div style={{fontFamily:"'Playfair Display',serif",fontSize:compact?19:24,fontWeight:700,lineHeight:1.2}}>You're {fmtM(wc.gap)} short of your goal — here's the path to close it</div>
      </div>
      <div style={{fontFamily:"'Nunito',sans-serif",fontSize:13,opacity:0.92,lineHeight:1.6,marginBottom:14,maxWidth:680}}>Your projected resources fall short of fully funding your desired lifestyle. This is the gap to close before exit — and it's closeable. The most common levers:</div>
      <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
        {levers.map((l,i)=><div key={i} style={{background:"rgba(255,255,255,0.16)",borderRadius:10,padding:"9px 14px",fontFamily:"'Nunito',sans-serif",fontSize:12.5,fontWeight:600}}>{l}</div>)}
      </div>
      {!compact&&<div style={{fontFamily:"'Nunito',sans-serif",fontSize:12,opacity:0.82,marginTop:14}}>Use the "what if I wait?" tool in the Exit Strategy tab to test the timing lever live.</div>}
    </div>
  </div>);
}
...
function ModuleBusiness({biz,setBiz,wealth,setWealth,overrides,setOverrides,kData,isAdvisor}){
  ...
  <div style={{display:"flex",flexDirection:"column",gap:24}}>
    ...
    <div style={{display:"flex",flexWrap:"wrap",gap:16}}>{industryInputs(biz.industry).map(f=><Field key={f.key} label={f.label} half><Input value={biz[f.key]||""} onChange={v=>sb(f.key,v)} placeholder={f.ph} type="number" hint={f.hint}/></Field>)}</div></Card>}
...
function ModuleContingency({data,set,biz}){
  const s=(k,v)=>set(d=>({...d,[k]:v}));
  const flags=[];
  if(!data.buySell||data.buySell==="no") flags.push({level:"critical",msg:"No Buy-Sell Agreement. Unexpected death or disability creates major legal and financial risk for co-owners and family."});
  if(!data.keyManIns||data.keyManIns==="no") flags.push({level:"critical",msg:"No Key Man Insurance. Suggest coverage equal to 1–2x annual revenue to protect business continuity."});
  if(!data.succession||data.succession==="no") flags.push({level:"warning",msg:"No formal succession plan. Identify and begin preparing internal candidates."});
  if(data.equalOwners==="yes"&&(!data.buySell||data.buySell==="no")) flags.push({level:"critical",msg:"Equal partners with no Buy-Sell Agreement — flag for attorney immediately. Unusually high risk."});
  if(!data.will||data.will==="no") flags.push({level:"warning",msg:"No will in place. Estate documents are foundational to any exit plan."});
  if(biz.structure==="c_corp") flags.push({level:"warning",msg:"C-Corp structure — discuss S-Corp conversion with CPA. 5-year window before sale recommended to avoid double taxation."});
  return(<div><SectionHeader num="4" title="Contingency & Risk Planning" subtitle="The 'break glass' plan — what happens if the owner dies unexpectedly? Insurance gaps and legal flags for the attorney."/>
  <div style={{display:"flex",flexDirection:"column",gap:24}}>
    <Card><h3 style={{fontFamily:"'Playfair Display',serif",fontSize:17,color:C.navy,marginTop:0,marginBottom:20}}>Ownership & Legal Structure</h3>
    <div style={{display:"flex",flexWrap:"wrap",gap:16}}>
      <Field label="Number of Owners" third><Input value={data.numOwners||""} onChange={v=>s("numOwners",v)} placeholder="1" type="number"/></Field>
      <Field label="Equal Ownership Split?" third><Select value={data.equalOwners||"no"} onChange={v=>s("equalOwners",v)} options={[{value:"no",label:"No — unequal"},{value:"yes",label:"Yes — equal partners"}]}/></Field>
      ...
      <Field label="Buy-Sell Funding" half><Select value={data.buySellFunding||"none"} onChange={v=>s("buySellFunding",v)} options={[{value:"none",label:"None"},{value:"life_ins",label:"Life insurance funded"},{value:"installment",label:"Installment payments"},{value:"sinking_fund",label:"Sinking fund"}]}/></Field>
      ...
      <Field label="Trust in Place?" third><Select value={data.trust||"no"} onChange={v=>s("trust",v)} options={[{value:"no",label:"No"},{value:"yes",label:"Yes"},{value:"outdated",label:"Outdated"}]}/></Field>
      ...
      <Field label="Last Beneficiary Update" third><Input value={data.lastBene||""} onChange={v=>s("lastBene",v)} placeholder="Year" type="number"/></Field>
    </div></Card>
    <Card><h3 style={{fontFamily:"'Playfair Display',serif",fontSize:17,color:C.navy,marginTop:0,marginBottom:20}}>Insurance</h3>
    <div style={{display:"flex",flexWrap:"wrap",gap:16}}>
      <Field label="Key Man Life Insurance?" half><Select value={data.keyManIns||"no"} onChange={v=>s("keyManIns",v)} options={[{value:"no",label:"No"},{value:"yes",label:"Yes — adequate"},{value:"yes_low",label:"Yes — likely underfunded"}]}/></Field>
      <Field label="Current Key Man Coverage" half><Input value={data.keyManAmount||""} onChange={v=>s("keyManAmount",v)} prefix="$" placeholder="0" type="number"/></Field>
      ...
      <Field label="Long-Term Care Insurance?" half><Select value={data.ltcIns||"no"} onChange={v=>s("ltcIns",v)} options={[{value:"no",label:"No"},{value:"yes",label:"Yes"},{value:"reviewing",label:"Reviewing"}]}/></Field>
    </div>
    {p(biz.revenue)>0&&<div style={{marginTop:8,padding:"14px 18px",borderRadius:10,background:C.cream,border:`1px solid ${C.border}`}}><p style={{fontFamily:"'Nunito',sans-serif",fontSize:13,color:C.slate,margin:0}}><strong>Suggested Key Man Coverage:</strong> {fmt(p(biz.revenue)*1.5)} – {fmt(p(biz.revenue)*2.0)} (1.5–2× annual revenue of {fmt(p(biz.revenue))})</p></div>}
    </Card>
    <Card><h3 style={{fontFamily:"'Playfair Display',serif",fontSize:17,color:C.navy,marginTop:0,marginBottom:20}}>Succession Planning</h3>
    <div style={{display:"flex",flexWrap:"wrap",gap:16}}>
      ...
      <Field label="Key Employees Who Must Be Retained"><Textarea value={data.keyRetention||""} onChange={v=>s("keyRetention",v)} placeholder="Names / roles who carry significant client or operational value..." rows={3}/></Field>
    </div></Card>
    {flags.length>0&&<Card style={{background:"rgba(192,57,43,0.04)",border:"1px solid rgba(192,57,43,0.25)"}}><h3 style={{fontFamily:"'Playfair Display',serif",fontSize:17,color:C.red,marginTop:0,marginBottom:16}}>⚠ Legal Flags — Advisor Action Items</h3><p style={{fontFamily:"'Nunito',sans-serif",fontSize:13,color:C.slate,marginBottom:16,marginTop:0}}>Share with client's attorney before document drafting. These items are specific to this client's situation.</p><div style={{display:"flex",flexDirection:"column",gap:10}}>{flags.map((f,i)=><div key={i} style={{padding:"12px 16px",borderRadius:8,background:f.level==="critical"?"rgba(192,57,43,0.08)":"rgba(215,119,6,0.08)",border:`1px solid ${f.level==="critical"?"rgba(192,57,43,0.3)":"rgba(215,119,6,0.3)"}`,display:"flex",gap:10}}><span style={{fontSize:16,flexShrink:0}}>{f.level==="critical"?"🔴":"🟡"}</span><p style={{fontFamily:"'Nunito',sans-serif",fontSize:13,color:C.slate,margin:0,lineHeight:1.6}}>{f.msg}</p></div>)}</div></Card>}
  </div></div>);}
  ...
  const printSummary=()=>{
    ...
    const body=`
      ...
      <div style="padding:28px 32px;font-family:Nunito,Arial,sans-serif;color:#2d2d2d">
        ...
        ${goalsReady?`<div style="border-radius:10px;padding:16px 18px;margin-bottom:22px;background:${wc.gap<=0?"#e8f5ee":"#eaf0fb"};border-left:5px solid ${wc.gap<=0?"#0f7a52":"#063894"}"><div style="font-family:'Playfair Display',Georgia,serif;font-size:17px;font-weight:700;color:${wc.gap<=0?"#0f7a52":"#063894"}">${wc.gap<=0?"On track — fully funded":"You're "+fmtM(wc.gap)+" short of your goal — and it's closeable"}</div><div style="font-size:12.5px;color:#2d2d2d;margin-top:5px;line-height:1.5">${wc.gap<=0?"Your projected resources fully fund your desired lifestyle, with a surplus of "+fmtM(Math.abs(wc.gap))+".":"Closeable by saving about "+fmt(wc.gap/Math.max(1,wc.yrs))+"/yr more, growing the business so the sale nets more, or adjusting the exit timing."}</div></div>`:""}
